...
def create_grid(
    ...
    Gaussian-smoothed random arrays produce realistic patchy fuel distributions and gradual terrain rather than
    independent per-node noise. Non-burnable patches (representing water bodies and rock) are generated by
    thresholding a third smoothed field, producing spatially coherent patches with fuel=0.
    ...
    :param nonburnable_fraction: Approximate fraction of nodes to mark as non-burnable (fuel=0), representing water
        and rock. A smoothed random field is thresholded to produce spatially coherent patches. Default 0.0 produces
        no non-burnable patches.
    ...
    if nonburnable_fraction > 0.0:
        nonburnable = _normalize(gaussian_filter(rng.random((rows, cols)), sigma=smoothing))
        fuel_norm[nonburnable < nonburnable_fraction] = 0.0
