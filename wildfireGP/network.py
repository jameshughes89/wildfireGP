...
slope : float in [0, 1]
    Terrain steepness normalised to [0, 1]. Fire spread rate increases with slope (Rothermel, 1972). Currently generated
    synthetically; will be derived from a DEM via numpy.gradient when real data loading is added.
...
Synthetic data
--------------
create_grid() generates a synthetic landscape using spatially correlated random fields (Gaussian-smoothed noise) for
both fuel and slope. The smoothing parameter controls patch size, producing landscapes with realistic spatial structure
rather than independent per-node noise.
...
def create_grid(rows: int, cols: int, smoothing: float = 3.0, seed: int | None = None) -> nx.Graph:
    ...
    :param smoothing: Gaussian filter sigma. Higher values produce smoother, larger-scale spatial patterns.
        Default 3.0 gives patch sizes of roughly 3 nodes.
    :param seed: Random seed for reproducibility.
    :return: Grid graph with STATE, FUEL, and SLOPE node attributes. Wind and moisture are not set; call
