...
This module defines the graph contract used throughout wildfireGP. A landscape is
represented as a NetworkX grid graph where each node corresponds to a spatial patch
and carries three attributes: state, fuel, and slope. Fire weather (wind and moisture)
is stored as graph-level attributes, as these quantities are meteorologically driven
and effectively uniform across the landscape at the scales we model.

Node attributes
---------------
state : NodeState
    Dynamic fire state of the patch (UNBURNED, BURNING, BURNED, or TREATED).
    Initialised to UNBURNED; modified by the spread simulation.
fuel : float in [0, 1]
    Relative fuel load. Higher values indicate more burnable material and
    increase ignition probability and burn duration. On real landscapes this
...
----------------------
wind_speed : float
    Wind speed in km/h. Wind is the dominant driver of fire spread direction
    and rate (Rothermel, 1972). Modelled as a uniform field — per-node variation
    would require meteorological downscaling data not generally available.
wind_direction : float
    Wind direction in degrees (0 = north, clockwise). Uniform across the landscape.
fuel_moisture : float in [0, 1]
    Relative fuel moisture. Acts as an ignition gate: high moisture suppresses
    spread, low moisture amplifies it. In practice this corresponds to the Fine
    Fuel Moisture Code (FFMC) from the Canadian Fire Weather Index system
    (Van Wagner, 1987).
...
- Canopy attributes (height, bulk density, base height, cover): relevant for
  crown fire modelling. We model surface fire only, which is appropriate for
  the GP resource allocation framing. Adding canopy attributes would require
  3+ additional node features with marginal benefit at this scale.
- Aspect: compass direction a slope faces. A second-order microclimate effect
  dominated by wind direction and slope magnitude at the scales we model.
- Spotting (ember transport): long-range stochastic ignition ahead of the fire
  front. Requires a separate sub-model and is out of scope for a graph-based CA.
- Dynamic fuel moisture: temporal variation in moisture during a single run.
  Held constant per run, consistent with standard probabilistic CA practice.
- Particle-level fuel properties (heat content, particle size, mineral content):
  Rothermel uses 13 fuel descriptors; categorical fuel type captures sufficient
  variance for our purposes.

Real data loading
-----------------
from_fbp_raster() loads Canadian landscape data:
    Fuel: FBP Fuel Types GeoTIFF from NRCan/CWFIS
          https://cwfis.cfs.nrcan.gc.ca/datamart
    DEM:  Canadian Digital Elevation Model (NRCan) or SRTM

    Default integer codes follow the NRCan FBP Fuel Types product
    (v20191114). Verify against your specific data product metadata and
    override fuel_map if codes differ.

from_landfire_raster() loads US landscape data:
    Fuel: LANDFIRE Scott-Burgan 40 fuel model GeoTIFF
          https://landfire.gov/data
    DEM:  LANDFIRE elevation layer or SRTM

Both functions require rasterio. No GIS software is needed. The graph is
constructed as nx.grid_2d_graph with node (i, j) corresponding to raster
pixel (row i, col j).

Synthetic data
--------------
create_grid() generates a synthetic landscape using spatially correlated
random fields (Gaussian-smoothed noise) for both fuel and slope. The smoothing
parameter controls patch size, producing landscapes with realistic spatial
structure rather than independent per-node noise.
...
Pais, C. et al. (2021). Cell2Fire: A Cell-Based Forest Fire Growth Model to
    Support Strategic Landscape Management Planning. Frontiers in Forests
    and Global Change. https://doi.org/10.3389/ffgc.2021.692706
"""
...
def create_grid(rows: int, cols: int, smoothing: float = 3.0, seed: int | None = None) -> nx.Graph:
    ...
    Gaussian-smoothed random arrays produce realistic patchy fuel distributions
    and gradual terrain rather than independent per-node noise.
    ...
    rows, cols : int
        Grid dimensions.
    smoothing : float
        Gaussian filter sigma. Higher values produce smoother, larger-scale
        spatial patterns. Default 3.0 gives patch sizes of roughly 3 nodes.
    seed : int or None
    ...
    nx.Graph
        ...
        Wind and moisture are not set; call set_wind() and
        set_fuel_moisture() before running simulations.
    """
...
def from_fbp_raster(
    ...
    fuel_map : dict or None
        Mapping from FBP integer code to float [0, 1]. Defaults to
        FBP_FUEL_MAP. Unknown codes map to 0.0 (non-fuel).
...
def from_landfire_raster(
    ...
    fuel_map : dict or None
        Mapping from Scott-Burgan integer code to float [0, 1]. Defaults to
        SCOTT_BURGAN_FUEL_MAP. Unknown codes map to 0.0 (non-burnable).
...
def reset_states(graph: nx.Graph) -> None:
    ...
    Call between GP fitness evaluations to reuse the same graph without
    rebuilding it.
    """
